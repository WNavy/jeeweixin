...
public class MessageReceiver_Listener {
	...
	// tcp 地址 服务器器端地址
	public static final String BROKER_URL = ActiveMQConnection.DEFAULT_BROKER_URL; // 其值为
																					// "tcp://localhost:61616";
	// 目标地址，在ActiveMQ管理员控制台创建
	// http://localhost:8161/admin/queues.jsp中可以查询到发送的mq消息
	...
	public static void run() throws Exception {
		...
		try {
			...
			ConnectionFactory factory = new ActiveMQConnectionFactory(
					MessageReceiver_Listener.DEFAULT_USER,
					MessageReceiver_Listener.DEFAULT_PASSWORD,
					MessageReceiver_Listener.BROKER_URL);
			// 2、通过工厂创建一个连接
