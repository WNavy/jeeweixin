...
import com.cache.redis.CacheUtilImpl;
import com.core.datasource.DataSource;
...
public class MsgNewsServiceImpl implements MsgNewsService{
	...
	@Autowired
	private CacheUtilImpl cacheutil;
//	@Autowired
...
//		String ss=demoService.returnHello();
		if(cacheutil.get("MsgNews", list.getClass())!=null){
			list=cacheutil.get("MsgNews", list.getClass());
		}else{
			list=entityDao.listForPage(searchEntity);
			cacheutil.put("MsgNews", list);
		}
		return list;
	}
